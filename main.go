...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	return (CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift) * r.Weight / float64(MInKm) * r.Duration.Hours() * float64(MinInHours)
}
...
func (w Walking) Calories() float64 {
	...
	return (CaloriesWeightMultiplier*w.Training.Weight + (math.Pow(w.meanSpeed()*KmHInMsec, 2)/w.Height/float64(CmInM))*CaloriesSpeedHeightMultiplier*w.Training.Weight) * w.Training.Duration.Hours() * float64(MinInHours)
}
...
func (s Swimming) meanSpeed() float64 {
	...
	return float64(s.LengthPool) * float64(s.CountPool) / float64(MInKm) / s.Training.Duration.Hours()
}
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	return (s.meanSpeed() + SwimmingCaloriesMeanSpeedShift) * float64(SwimmingCaloriesWeightMultiplier) * s.Training.Weight * s.Training.Duration.Hours()
}
...
func (s Swimming) TrainingInfo() InfoMessage {
	...
	return InfoMessage{
		...
		Duration:     s.Duration,
		Distance:     float64(s.LengthPool) * float64(s.CountPool) / float64(MInKm),
		Speed:        s.meanSpeed(),
		Calories:     s.Calories(),
	}
}
